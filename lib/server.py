...
class Server:
    ...
    def handle_client_gbn(self, client_address, client_queue, window: Window):
        ...
        while True:
            ...
            if packet.get_fin():
                log.debug('Se recibe un paquete de fin de conexion')
                self.send_gbn(
                    client_address,
                    Packet(packet.seq_num + 1, end_conection=True, ack=True),
                    window,
                )
                log.debug('Esperando un segundo por si no recibe el ack antes '
                          'de cerrar la conexion')
                time.sleep(1)
                while not client_queue.empty():
                    packet = client_queue.get_nowait()
                    if packet.get_fin():
                        self.send_gbn(client_address,
                                      Packet(packet.seq_num + 1, end_conection=True, ack=True),
                                      window)
    ...
    def cleanup_clients(self):
        ...
        for client_address in to_remove:
            ...
            log.debug(f"Contexto limpiado para el cliente {client_address}")
    ...
    def send_gbn(self, client_address, packet, window):
        while True:
            if packet.ack or window.try_add_packet(packet):
                ...
                # Espera activa
                log.debug(f"Ventana llena, esperando para enviar paquete {packet.seq_num} "
                          f"a {client_address}")
