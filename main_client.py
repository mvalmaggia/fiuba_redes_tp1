...
def main():
    parser = argparse.ArgumentParser(description="Upload or download files from a server")
    # El comando es requerido
    parser.add_argument("command", help="Command to execute", choices=["upload", "download"], type=str)
    ...
    parser.add_argument("-H", "--host", help="Host address", required=True, type=str)
    parser.add_argument("-p", "--port", help="Port number", required=True, type=int)
    parser.add_argument("-s", "--file_path", help="File path", required=True, type=str)
